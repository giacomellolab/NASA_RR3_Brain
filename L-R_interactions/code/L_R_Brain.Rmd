...
seurat_multi <- readRDS(file = path1)
names(seurat_multi[[]])
...
brain_LR <- subset(seurat_multi, idents = c('4', '5', '11', '14')) #we took the multiomics clusters with the largest spaceflight changes based on DEGs results
...
brain_LR_metadata$cell_type[which(brain_LR_metadata$cell_type == '4')] <- 'multiomics cluster 4'
brain_LR_metadata$cell_type[which(brain_LR_metadata$cell_type == '5')] <- 'multiomics cluster 5'
brain_LR_metadata$cell_type[which(brain_LR_metadata$cell_type == '11')] <-'multiomics cluster 11'
brain_LR_metadata$cell_type[which(brain_LR_metadata$cell_type == '14')] <-'multiomics cluster 14'
...
DE_genes_SC <- data.frame() # Create empty data.frame
  excel_sheet <- read.xlsx(paste0(path1,"formatted_de_genes_2022_06-09.xlsx"), colNames  = TRUE, sheet = 2) # Read sheet i, these are DEGs list from the supplementary data multiomics but for 4, 5, 1, 14
  # the row values being used here will be different depending on the indexes for the DEGs in your excel sheet
  DE_genes_SC <- rbind(DE_genes_SC, excel_sheet) 
  Cl4 <-  DE_genes_SC[1:93,1:4] #DEGs for multiomics cl 4
  colnames(Cl4) <-  Cl4[1,]
  Cl4 <- Cl4[-1,]
 Cl4$cluster <- '4'
 Cl4 <- Cl4[,c(5,1:4)]
 
   Cl5 <-  DE_genes_SC[1:171,5:8] #DEGs for multiomics cl 5
  colnames(Cl5) <-  Cl5[1,]
  Cl5 <- Cl5[-1,]
   Cl5$cluster <- '5'
 Cl5 <- Cl5[,c(5,1:4)]
  
   Cl11 <-  DE_genes_SC[1:47,17:20] #DEGs for multiomics cl 11
  colnames(Cl11) <-  Cl11[1,]
  Cl11 <- Cl11[-1,]
   Cl11$cluster <- '11'
 Cl11 <- Cl11[,c(5,1:4)]
  
    Cl14 <-  DE_genes_SC[1:28,21:24] #DEGs for multiomics cl 4
...
```{r}
brain2$cluster[which(brain2$cluster == '4')] <- 'multiomics cluster 4'
brain2$cluster[which(brain2$cluster == '5')] <- 'multiomics cluster 5'
brain2$cluster[which(brain2$cluster == '11')] <-'multiomics cluster 11'
brain2$cluster[which(brain2$cluster == '14')] <-'multiomics cluster 14'
...
#this is to run cellphoneDB plot
...
Recommendation from CellPhoneDB authors:
...
# DotPlot for relevant interactions flight upregulated ligand_recceptor pairs (multiomics cluster 4, 11 considered only)

Discussion on this in the manuscript file and the peer-review file.
